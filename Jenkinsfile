...
def deploy () {
    ...
    withCredentials([string(credentialsId: "sbr-api-dev-secret-key", variable: 'APPLICATION_SECRET')]) {
        deployToCloudFoundryHBase("cloud-foundry-sbr-${env.DEPLOY_NAME}-user", 'sbr', "${env.DEPLOY_NAME}", "${env.DEPLOY_NAME}-pipeline", "${env.DEPLOY_NAME}-ons-sbr-pipeline.zip", "conf/${env.DEPLOY_NAME}/manifest.yml", "tbl", "namespace")
    }
}
